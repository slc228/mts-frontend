<template>
  <!--<Loading></Loading>-->
  <div style="width: 100%; height: 100%" id="emotionMountCanvas"></div>
</template>  
<script>
//通过this.$echarts来使用
...
export default {
  ...
  setup(props, context) {
    return {
      resize(e) {
        setTimeout(() => {
          let eChart = echarts.getInstanceByDom(document.getElementById("emotionMountCanvas"));
          if (eChart != null) {
            eChart.resize();
          }
        }, 500);
      ...
      load() {
        this.eChart = echarts.getInstanceByDom(document.getElementById("emotionMountCanvas"));
        if (this.eChart != null) {
          this.resize();
        } else {
          this.init("", "");
      ...
      initEChart(data, planId) {
        ...
        for (let item of data) {
          ...
          source[i].push(item.happy);
          source[i].push(item.neutral);
          source[i].push(item.surprise);
          source[i].push(item.sad);
          source[i].push(item.angry);
          source[i].push(item.fear);
        ...
        if (this.eChart == null) {
          this.eChart = echarts.init(
            document.getElementById("emotionMountCanvas")
          );
        }
        this.eChart.off('click');
        this.eChart.on('click', function (params) {
          window.location.href = '?page=plan&category=emotionmount&key=' + planId;
        });
      ...
      initOption(title, series, source) {
        return {
          title: {
            text: props.DarkMode ? "" : title + " 情感趋势分析",
            //   subtext: "Fake Data",
            left: "center",
          },
          animation: true,
          ...
          tooltip: {
            ...
            formatter: function (x) {
              console.log(x);
              return x.seriesName + " " + x.data[0] + " [" + x.data[x.componentIndex + 1] + "条]";
            },
            textStyle: {
              align: "left",
              fontSize: 15,
          ...
          legend: {
            //  orient: "vertical",
            data: ["积极", "中立", "震惊", "悲伤", "愤怒", "恐惧", "总计"],
            left: "center",
            type:"scroll",
            top: props.DarkMode ? 0 : 35,
            textStyle: {
              //    size: "14px",
              fontSize: 10,
