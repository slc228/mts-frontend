<template>
  <!--<Loading></Loading>-->
  <div style="width: 100%; height: 100%" id="sourceCanvas"></div>
</template>  
<script>
//通过this.$echarts来使用
...
export default {
  ...
  setup(props, context) {
    return {
      resize(e) {
        setTimeout(() => {
          let eChart = echarts.getInstanceByDom(document.getElementById("sourceCanvas"));
          if (eChart != null) {
            eChart.resize();
          }
        }, 500);
      ...
      load() {
        this.eChart = echarts.getInstanceByDom(document.getElementById("sourceCanvas"));
        if (this.eChart != null) {
          this.resize();
        } else {
          this.init("", "");
      ...
      initEChart(data, planId) {
        ...
        for (let item of data) {
          ...
          source[i].push(((item.value * 100) / total).toFixed(2));
        ...
        if (this.eChart == null) {
          this.eChart = echarts.init(document.getElementById("sourceCanvas"));
        }
        this.eChart.off('click');
        this.eChart.on('click', function (params) {
          window.location.href = '?page=plan&category=source&key=' + planId;
        });
      ...
      initOption(title, series, source) {
        return {
          title: {
            text: props.DarkMode ? "" : title + " 来源分布分析",
            //   subtext: "Fake Data",
            left: "center",
          },
          animation: true,
          ...
          toolbox: {
            ...
            feature: {
              ...
              /*  importButton: {//自定义按钮 danielinbiti,这里增加，selfbuttons可以随便取名字  
                            ...
                            icon: 'image://../yuqing/image/Icon/import.png', //图标  
